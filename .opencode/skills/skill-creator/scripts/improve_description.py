...
def _call_claude(prompt: str, model: str | None, timeout: int = 300) -> str:
    """Run `opencode run` with the prompt as an argument and return the text response.

    Pass prompt in arguments because `opencode run` doesn't currently pipe well with large texts through stdin.
    """
    ...
    # Remove OPENCODE env vars to allow nesting opencode run inside a
...
def improve_description(
    ...
    prompt = f"""You are optimizing a skill description for a Opencode Code skill called "{skill_name}". A "skill" is sort of like a prompt, but with progressive disclosure -- there's a title and description that Opencode sees when deciding whether to use the skill, and then if it does use the skill, it reads the .md file which has lots more details and potentially links to other resources in the skill folder like helper files and scripts and additional documentation or examples.
